...
PirQuery pir_client::generate_query_combined64(uint64_t desiredIndex) {
    ...
    for (uint32_t i = 0; i < 1; i++) {
        ...
        for (uint32_t j =0; j < num_ptxts; j++){
            ...
            packed_ct.clear();
            poc_plain_gsw_enc64_combined(decomp_size, base_bits, newcontext_, sk, packed_ct, pt, pool, pir_params_.nvec[i], gap);
...
//            cout<<ppt.to_string()<<endl;
    ...
    if(indices_.size()>1) {
        ...
        base_bits = pir_params_.gsw_base;
        decomp_size = pir_params_.gsw_decomp_size;
        poc_half_gsw_enc64_combined(decomp_size, base_bits, newcontext_, sk, half_gsw_ciphertext, pt, pool, (1 << logsize), gap, dimension_size);
...
Plaintext pir_client::decrypt_result(std::vector<seal::Ciphertext> reply) {
        ...
        Plaintext ppt;
        //evaluator_->mod_switch_to_next_inplace(reply[0]);

        decryptor_->decrypt(reply[0],ppt);

        cout<<"Client: Noise in output = "<<decryptor_->invariant_noise_budget(reply[0])<<endl;
        return ppt;
...
GSWCiphertext pir_client::get_enc_sk() {
    ...
    poc_enc_sk_gsw64(keygen_->secret_key(),newcontext_,pir_params_.secret_base, sk_gsw_ciphertext);
