        ...
        for (uint64_t k = 0; k < product; k++) {
            ...
            for (uint64_t j = 1; j < n_i; j++) {
                ...
                //poc_nfllib_add_ct(first_dim_intermediate_cts[k], temp,newcontext_);
                 ...
                 durrr =   duration_cast<microseconds>(expand_end - expand_start).count();
...
PirReply pir_server::generate_reply_combined64(PirQuery query, uint32_t client_id, SecretKey sk) {
    ...
    for (uint32_t i = 0; i < 1; i++) {
        ...
        for (uint64_t k = 0; k < product; k++) {
            ...
            for (uint64_t j = 1; j < n_i; j++) {
                ...
                //poc_seal_external_product(list_enc[j], split_db[k + j * product], newcontext_, decomp_size, temp,1);
                auto mul_end  = high_resolution_clock::now();

                time = duration_cast<microseconds>(mul_end - mul_start).count();
                //cout << "Time per mul: " << time << " us" << endl;
        ...
        int duration =duration_cast<milliseconds>(expand_end - expand_start).count();
        cout << "Server: first-dimension dot-prod cost = " << duration << " ms" << endl;
        ...
        expand_end  = high_resolution_clock::now();
        duration =  duration_cast<milliseconds>(expand_end - expand_start).count();
        cout << "Server: INTT after first dimension = " << duration << " ms" << endl;
...
//        logsize = ceil(log2(new_dimension_size));
    ...
    gsw_server_expand_combined64_nocrt(gswCiphers_ptr, query[1], newcontext_, 0, decomp_size, total_dim_size, galoisKeys_,  decomp_size, pir_params_.gsw_base, sk_decomp_size, pir_params_.secret_base, sk_enc_, 1<<logsize);
    ...
    for (uint32_t i = 1; i < nvec.size(); i++){
        ...
        for (uint64_t k = 0; k < product; k++) {
            ...
            intermediateCtxts[k].resize(newcontext_, newcontext_->first_context_data()->parms_id(), 2);
            vector<uint64_t *> rlwe_decom;
            rwle_decompositions64_nocrt(first_dim_intermediate_cts[k], newcontext_, decomp_size, pir_params_.gsw_base, rlwe_decom);
            poc_nfllib_ntt_rlwe_decomp(rlwe_decom);
            poc_nfllib_external_product(CtMuxBits[0 + previous_dim], rlwe_decom, newcontext_, decomp_size, intermediateCtxts[k],1);
            for (auto p : rlwe_decom) {
                free(p);
            ...
            for (uint64_t j = 1; j < n_i; j++) {
                ...
                Ciphertext temp;
                rlwe_decom.clear();
                rwle_decompositions64_nocrt(first_dim_intermediate_cts[k + j * product], newcontext_, decomp_size, pir_params_.gsw_base, rlwe_decom);
                poc_nfllib_ntt_rlwe_decomp(rlwe_decom);
                temp.resize(newcontext_, newcontext_->first_context_data()->parms_id(), 2);
                poc_nfllib_external_product(CtMuxBits[j + previous_dim], rlwe_decom, newcontext_, decomp_size, temp,1);
...
void pir_server::preprocess_database64() {
    if (!is_db_preprocessed_) {
        ...
        for (uint32_t i = 0; i < db_->size(); i++) {
            ...
            vector<uint64_t *> plain_decom;
            plain_decompositions64_nocrt(db_->data()[i], newcontext_, decomp_size, pir_params_.plain_base, plain_decom);
            poc_nfllib_ntt_rlwe_decomp(plain_decom);
            split_db.push_back(plain_decom);
