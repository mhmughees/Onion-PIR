...
void poc_gsw_enc64(const uint64_t l, const uint64_t base_bit, shared_ptr<SEALContext> context, const SecretKey sk,
                   vector<Ciphertext> &gsw_ciphertext, Plaintext gsw_plain, seal::util::MemoryPool &pool,
                   uint64_t inv) {
    Encryptor encryptor(context, sk);
    Decryptor decryptor(context, sk);
    ...
    for (int j = 0; j < 2; j++) {// c0, c1
        ...
        for (int p = 0; p < r_l; p++) {
            ...
            poc_multiply_add_plain_without_scaling_variant64(gsw_plain, *context->first_context_data(), shift_amount,
                                                             res.data(j), pool, inv);
...
void
poc_multiply_add_plain_without_scaling_variant(const Plaintext &plain, const SEALContext::ContextData &context_data,
...
void
poc_multiply_add_plain_without_scaling_variant64(const Plaintext &plain, const SEALContext::ContextData &context_data,
                                                 const int shift_amount, uint64_t *destination,
                                                 seal::util::MemoryPool &pool, uint64_t inv = 0) {
    auto &parms = context_data.parms();
    size_t coeff_count = parms.poly_modulus_degree();
    ...
    for (size_t i = 0; i < plain_coeff_count; i++) {
        ...
        for (size_t j = 0; j < coeff_mod_count; j++) {
            ...
            uint64_t result = 0;
            //use 128 bit implementation for left shifts 1<<shiftamount
            util::left_shift_uint(&ptr, shift_amount, 1, &result);
            h = seal::util::barrett_reduce_63(result, coeff_modulus[j]);
...
void
rwle_decompositions64(Ciphertext rlwe_ct_1, shared_ptr<SEALContext> context, const uint64_t l, const uint64_t base_bit,
                      vector<uint64_t *> &rlwe_decom) {
    const auto &context_data2 = context->first_context_data();
    auto &parms2 = context_data2->parms();
    auto &coeff_modulus = parms2.coeff_modulus();
    ...
    poc_decomp_rlwe64(rlwe_ct_1, l, context, rlwe_decom, base_bit, pool);
...
void
rwle_decompositions64_nocrt(Ciphertext rlwe_ct_1, shared_ptr<SEALContext> context, const uint64_t l,
                            const uint64_t base_bit,
                            vector<uint64_t *> &rlwe_decom) {
    const auto &context_data2 = context->first_context_data();
    auto &parms2 = context_data2->parms();
    auto &coeff_modulus = parms2.coeff_modulus();
    ...
    poc_decomp_rlwe64_nocrt(rlwe_ct_1, l, context, rlwe_decom, base_bit, pool);
...
void poc_decomp_plain(Plaintext pt, const uint64_t decomp_size, shared_ptr<SEALContext> context,
    ...
    for (int j = 0; j < pt_poly_count; j++) {// c0, c1
        ...
        for (int p = 0; p < r_l; p++) {
            ...
            res = (std::uint64_t *) calloc((coeff_count * coeff_mod_count),
                                           sizeof(uint64_t)); //we are allocating larger space to cater for ct modulus later
            const int shift_amount = ((total_bits) - ((p + 1) * base_bit));
...
void poc_decomp_plain64(Plaintext pt, const uint64_t decomp_size, shared_ptr<SEALContext> context,
                        vector<uint64_t *> &vec_ciphertexts, int base_bit, seal::util::MemoryPool &pool) {
...
//                ptr[0] = 0;
                ...
                //seal::util::right_shift_uint(&raw_ptr[k],shift_amount,1,ptr.get());
                seal::util::right_shift_uint(&raw_ptr[k], shift_amount, 1, &ptr);
                uint64_t temp1 = ptr & mask;
                res[k * coeff_mod_count] = temp1;
...
void
plain_decompositions(Plaintext &pt, shared_ptr<SEALContext> &context, const uint64_t decomp_size,
                     const uint64_t base_bit,
...
void
plain_decompositions64(Plaintext &pt, shared_ptr<SEALContext> &context, const uint64_t decomp_size,
                       const uint64_t base_bit,
                       vector<uint64_t *> &plain_decom) {
    const auto &context_data2 = context->first_context_data();
    auto &parms2 = context_data2->parms();
...
//    context_data2->rns_tool()->base_q()->compose_array(rlwe_ct_1.data(1), coeff_count, pool);
    ...
    poc_decomp_plain64(pt, decomp_size, context, plain_decom, base_bit, pool);
    ...
    for (int i = 0; i < ssize; i++) {
        ...
        //rwle_crt_decompose(rlwe_decom[i], context, pool);
        //cout<<i<<endl;
        poc_decompose_array64(plain_decom[i], coeff_count, coeff_modulus, coeff_modulus_size, pool);
...
void
plain_decompositions64_nocrt(Plaintext &pt, shared_ptr<SEALContext> &context, const uint64_t decomp_size,
                             const uint64_t base_bit,
                             vector<uint64_t *> &plain_decom) {
    const auto &context_data2 = context->first_context_data();
    auto &parms2 = context_data2->parms();
...
void poc_decomp_rlwe64(Ciphertext ct, const uint64_t l, shared_ptr<SEALContext> context,
                       vector<uint64_t *> &vec_ciphertexts, int base_bit, seal::util::MemoryPool &pool) {
...
//                ptr[1] = 0;
                ...
                uint64_t ptr = 0;
                seal::util::right_shift_uint(&encrypted1_ptr[k], shift_amount, 1, &ptr);
                //seal::util::right_shift_uint128(&encrypted1_ptr[k], shift_amount, ptr.get());
                uint64_t temp1 = ptr & mask;
                res[k] = temp1;
...
void poc_decomp_rlwe64_nocrt(Ciphertext ct, const uint64_t l, shared_ptr<SEALContext> context,
                             vector<uint64_t *> &vec_ciphertexts, int base_bit, seal::util::MemoryPool &pool) {
    ...
    for (int j = 0; j < ct_poly_count; j++) {// c0, c1
        ...
        for (int p = 0; p < r_l; p++) { // p in [0,l-1]
            vector<uint64_t *> results;
            res = (std::uint64_t *) calloc((coeff_count ), sizeof(uint64_t));
            const int shift_amount = ((total_bits) - ((p + 1) * base_bit));
            for (size_t k = 0; k <  coeff_count; k = k + 1) {
...
void poc_decompose_array64(uint64_t *value, size_t count, std::vector<Modulus> coeff_modulus, size_t coeff_mod_count,
                           MemoryPoolHandle pool) {
    if (!value) {
        throw invalid_argument("value cannot be null");
    }
    ...
    if (coeff_mod_count > 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            for (size_t j = 0; j < coeff_mod_count; j++) {
                // Reduce in blocks
                uint64_t result;
                uint64_t temp = temp_array[(i * coeff_mod_count) + coeff_mod_count - 1];
                for (size_t k = coeff_mod_count - 1; k--;) {
                    temp = temp_array[(i * coeff_mod_count) + k];
                    result = barrett_reduce_63(temp, coeff_modulus[j]);
                }

                // Save the result modulo i-th base element
                //value[i] = temp[1];
                value[(j * count) + i] = result;
            }
        }
    }
...
void poc_plain_gsw_enc64(const uint64_t decomp_size, const uint64_t base_bit, shared_ptr<SEALContext> context,
                         const SecretKey sk, vector<Ciphertext> &gsw_ciphertext, Plaintext gsw_plain,
                         seal::util::MemoryPool &pool, uint64_t inv) {
...
//    auto ptr2(allocate_uint(2, pool));
    ...
    for (int p = 0; p < r_l; p++) {
        ...
        result = 0;
        util::left_shift_uint(&ptr, shift_amount, 1, &result);
        h = seal::util::barrett_reduce_63(result, plain_modulus.value());
        if (inv > 0) {
            uint64_t tt;
            util::try_invert_uint_mod(inv, parms.plain_modulus().value(), tt);
            h = util::multiply_uint_uint_mod(h, tt, parms.plain_modulus());
...
void poc_plain_gsw_enc64_combined(const uint64_t decomp_size, const uint64_t base_bit, shared_ptr<SEALContext> context,
                                  const SecretKey sk, vector<Ciphertext> &gsw_ciphertext, Plaintext gsw_plain,
                                  seal::util::MemoryPool &pool, uint64_t inv, uint64_t gap) {
...
void poc_rlwe_expand_threaded(Ciphertext packedquery, shared_ptr<SEALContext> context, seal::GaloisKeys galkey,
                              uint64_t size, vector<Ciphertext> &result) {
...
vector<Ciphertext>
poc_rlwe_expand(Ciphertext packedquery, shared_ptr<SEALContext> context, seal::GaloisKeys galkey, uint64_t size) {
    /// this function return size  vector of RLWE ciphertexts
    /// it takes a single RLWE packed ciphertext
...
void poc_encrypt_gsw_sk(vector<Ciphertext> &sk_gsw_ciphertext, shared_ptr<SEALContext> context, SecretKey sk,
                        const int base_bit) {
    const auto &context_data2 = context->first_context_data();
    auto &parms2 = context_data2->parms();
    auto &coeff_modulus = parms2.coeff_modulus();
...
void poc_nfllib_external_product(vector<Ciphertext> &gsw_enc, vector<uint64_t *> &rlwe_expansion,
    ...
    size_t coeff_count = parms2.poly_modulus_degree();
    size_t coeff_mod_count = coeff_modulus.size();
    size_t encrypted1_size = gsw_enc[0].size();
    ...
    std::uint64_t *result = (std::uint64_t *) calloc((coeff_count * coeff_mod_count), sizeof(uint64_t));;
    Plaintext pt_tmp;
...
//            int duration = duration_cast<std::chrono::microseconds>(end - start).count();
            //std::cout << "Time per nfllib external mul : " << duration << " us, Size : " <<gsw_enc.size()*encrypted1_size  << std::endl;
...
void poc_seal_external_product(vector<Ciphertext> gsw_enc, vector<uint64_t *> rlwe_expansion,
                               shared_ptr<SEALContext> &context, int l, Ciphertext &res_ct, int is_reusable = 1) {
    ...
    std::uint64_t *result = (std::uint64_t *) calloc((coeff_count), sizeof(uint64_t));
    for (int k = 0; k < gsw_enc.size(); k++) {
        ...
        for (size_t j = 0; j < encrypted1_size; j++) {
            ...
            for (int i = 0; i < coeff_mod_count; i++) {
                uint64_t *result_copy = result;
                auto start = std::chrono::steady_clock::now();
                seal::util::dyadic_product_coeffmod(gsw_enc[k].data(j) + (i * coeff_count),
                                                    rlwe_expansion[k] + (i * coeff_count), coeff_count,
                                                    coeff_modulus[i], result_copy);
                auto end = std::chrono::steady_clock::now();
                int duration = duration_cast<std::chrono::microseconds>(end - start).count();
                std::cout << "Time per seal external mul : " << duration << " us, Size : "
                          << gsw_enc.size() * encrypted1_size << std::endl;

                seal::util::add_poly_poly_coeffmod(result, res_ct.data(j) + (i * coeff_count), coeff_count,
                                                   coeff_modulus[i], res_ct.data(j) + (i * coeff_count));
...
void ppoly_nfllib_prod_with_no_ntt(std::uint64_t *p1, std::uint64_t *p2, std::uint64_t *res, const size_t coeff_count,
    ...
    //std::cout << "Time per polynomial NTT: " << duration_cast<microseconds>(end - start).count() << " us" << std::endl;
...
void poc_expand_flat_threaded(vector<GSWCiphertext>::iterator &result, vector<Ciphertext> &packed_swap_bits,
    ...
    threads.emplace_back(
            std::thread(poc_rlwe_expand_threaded, packed_swap_bits[0], context, galkey, size, std::ref(expanded_t1)));
    threads.emplace_back(
            std::thread(poc_rlwe_expand_threaded, packed_swap_bits[1], context, galkey, size, std::ref(expanded_t2)));
    threads.emplace_back(
            std::thread(poc_rlwe_expand_threaded, packed_swap_bits[2], context, galkey, size, std::ref(expanded_t3)));
...
void poc_half_gsw_enc64(const uint64_t l, const uint64_t base_bit, shared_ptr<SEALContext> context, const SecretKey sk,
                        vector<Ciphertext> &gsw_ciphertext, Plaintext gsw_plain, seal::util::MemoryPool &pool,
                        uint64_t inv) {
    ...
    for (int p = 0; p < r_l; p++) {
        ...
        encryptor.encrypt_zero_symmetric(res);
        //set_ciphertext(res, context);
        mymultiply_add_plain_without_scaling_variant64(gsw_plain, *context->first_context_data(), shift_amount,
                                                       res.data(0), pool, inv);
        gsw_ciphertext.push_back(res);
    }
...
void poc_half_gsw_enc128_combined(const uint64_t l, const uint64_t base_bit, shared_ptr<SEALContext> context,
    ...
    for (int p = 0; p < r_l; p++) {
        ...
        mymultiply_add_plain_without_scaling_variant_combined(gsw_plain, *context->first_context_data(), shift_amount,
                                                              res.data(0), pool, total_expand_size, gap, dimension_size,
                                                              p);
...
void poc_half_gsw_enc64_combined(const uint64_t l, const uint64_t base_bit, shared_ptr<SEALContext> context,
                                 const SecretKey sk,
                                 vector<Ciphertext> &gsw_ciphertext, Plaintext gsw_plain, seal::util::MemoryPool &pool,
                                 uint64_t total_expand_size, uint64_t gap, uint64_t dimension_size) {
    ...
    for (int p = 0; p < r_l; p++) {
        ...
        //set_ciphertext(res, context);
        mymultiply_add_plain_without_scaling_variant_combined64(gsw_plain, *context->first_context_data(), shift_amount,
                                                                res.data(0), pool, total_expand_size, gap,
                                                                dimension_size,
                                                                p);
...
void mymultiply_add_plain_without_scaling_variant_combined(const Plaintext &plain,
                                                           const SEALContext::ContextData &context_data,
                                                           const int shift_amount, uint64_t *destination,
...
void mymultiply_add_plain_without_scaling_variant_combined64(const Plaintext &plain,
                                                             const SEALContext::ContextData &context_data,
                                                             const int shift_amount, uint64_t *destination,
                                                             seal::util::MemoryPool &pool, uint64_t total_expand_size,
                                                             uint64_t gap, uint64_t dimension_size,
...
void
mymultiply_add_plain_without_scaling_variant64(const Plaintext &plain, const SEALContext::ContextData &context_data,
                                               const int shift_amount, uint64_t *destination,
                                               seal::util::MemoryPool &pool, uint64_t inv) {
    auto &parms = context_data.parms();
    size_t coeff_count = parms.poly_modulus_degree();
...
void poc_enc_sk_gsw(SecretKey sk, shared_ptr<SEALContext> context, const int base_bit,
...
void poc_enc_sk_gsw64(SecretKey sk, shared_ptr<SEALContext> context, const int base_bit,
                      vector<Ciphertext> &sk_gsw_ciphertext) {
    ...
    //get gsw encrypting seckey
    poc_gsw_enc64_sk(l, base_bit, context, sk, sk_gsw_ciphertext, secret_key_pt, pool);
...
void thread_server_expand(vector<GSWCiphertext>::iterator &result, vector<Ciphertext> packed_swap_bits,
                          ...
                          const int l, const int base_bit, const int lsk, const int bsk,
                          vector<Ciphertext> sk_gsw_ciphertext2) {
...
void thread_server_expand64(vector<GSWCiphertext>::iterator &result, vector<Ciphertext> packed_swap_bits,
                            shared_ptr<SEALContext> context, int begin, int end, int size, seal::GaloisKeys galkey,
                            const int l, const int base_bit, const int lsk, const int bsk,
                            vector<Ciphertext> sk_gsw_ciphertext2) {
    ...
    for (int i = begin; i < end; i++) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            rlwe_decom.clear();
            rwle_decompositions64(expanded_ciphers[j], context, lsk, bsk, rlwe_decom);
...
void thread_server_expand64_nocrt(vector<GSWCiphertext>::iterator &result, vector<Ciphertext> packed_swap_bits,
                                  shared_ptr<SEALContext> context, int begin, int end, int size,
                                  seal::GaloisKeys galkey,
                                  const int l, const int base_bit, const int lsk, const int bsk,
                                  vector<Ciphertext> sk_gsw_ciphertext2, Decryptor &decryptor1) {
    ...
    for (int i = begin; i < end; i++) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            rlwe_decom.clear();
            rwle_decompositions64_nocrt(expanded_ciphers[j], context, lsk, bsk, rlwe_decom);
            poc_nfllib_ntt_rlwe_decomp(rlwe_decom);
            res_ct.resize(context, context->first_context_data()->parms_id(), 2);
            poc_nfllib_external_product(sk_gsw_ciphertext2, rlwe_decom, context, lsk, res_ct, 1);
            for (auto p : rlwe_decom) {
                free(p);
            }
...
//                cout<< "TTTTTT :" << ppt.to_string()<<endl;
//            }
...
void gsw_server_expand_combined64(vector<GSWCiphertext>::iterator &result, vector<Ciphertext> packed_swap_bits,
                                  shared_ptr<SEALContext> context, int begin, int end, int total_dim_size,
                                  seal::GaloisKeys galkey,
                                  const int l, const int base_bit, const int lsk, const int bsk,
                                  vector<Ciphertext> sk_gsw_ciphertext2,
    ...
    for (int i = begin; i < end; i++) {
        ...
        for (int j = 0; j < total_dim_size; j++) {
            ...
            rlwe_decom.clear();
            rwle_decompositions64(expanded_ciphers[j + i * total_dim_size], context, lsk, bsk, rlwe_decom);
...
void gsw_server_expand_combined64_nocrt(vector<GSWCiphertext>::iterator &result, vector<Ciphertext> packed_swap_bits,
                                  shared_ptr<SEALContext> context, int begin, int end, int total_dim_size,
                                  seal::GaloisKeys galkey,
                                  const int l, const int base_bit, const int lsk, const int bsk,
                                  vector<Ciphertext> sk_gsw_ciphertext2,
    ...
    for (int i = begin; i < end; i++) {
        ...
        for (int j = 0; j < total_dim_size; j++) {
            ...
            rlwe_decom.clear();
            rwle_decompositions64_nocrt(expanded_ciphers[j + i * total_dim_size], context, lsk, bsk, rlwe_decom);
...
void poc_gsw_enc64_sk(const uint64_t l, const uint64_t base_bit, shared_ptr<SEALContext> context, const SecretKey sk,
                      vector<Ciphertext> &gsw_ciphertext, Plaintext gsw_plain, seal::util::MemoryPool &pool) {
    Encryptor encryptor(context, sk);
    //Decryptor decryptor(context, sk);
    const auto &context_data = context->first_context_data();
...
void
mymultiply_add_plain_without_scaling_variant_sk64(const Plaintext &plain, const SEALContext::ContextData &context_data,
                                                  const int shift_amount, uint64_t *destination,
                                                  seal::util::MemoryPool &pool) {
